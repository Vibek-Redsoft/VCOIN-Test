...
function GraphBlock(props) {
    ...
    useEffect(() => {
        ...
        let aggregatedData = noAccumulation[graphTitle] ?
            graphSets : graphSets.map(accumulationMethod);
    ...
    useEffect(() => {
        ...
        chartSettings = !!pages && {
            series: [{
                ...
                // Y axis - array
                type: 'line',
                data: pages[currentPage].map(dataPoint => formatNumber(dataPoint.Value))
            }],
            options: {
                chart: {
                    type: 'line',
                ...
                labels: pages[currentPage].map(dataPoint => {
                    ...
                    let formatResult = groupType === 'week' ?
                        'Week ' + dateFormat[1] + ', ' + dateFormat[0]:
                        (dateFormat[2] ? ' ' + dateFormat[2] : '') + ' ' + GetMonthName[dateFormat[1] - 1] + ' ' + dateFormat[0]
    ...
    return (
        chart ?
            <div className="graph-block">
                ...
                <strong className="relative">{graphTitle} {<InfoIcon text={graphPopup} color="white" />}</strong>    
                </h4>
                <div className="block-chart">
                    <div className="menu flex ai-center">
                        <div className="page-position">
                            <p><span
                                className={ currentPage === pages.length - 1 ? 'end' : 'b' }>{ currentPage + 1 } /</span>
                                <span className="end">{ pages.length }</span></p>
                        </div>
                        <div className="controls">
                            <div className={ currentPage === pages.length - 1 ? 'end right' : "right" }
                                 onClick={ () => handleClick(1) }>
                                <picture>
                                    <source srcSet="images/graph-arrow.svg" type="image/svg"/>
                                    <img src="images/graph-arrow.png" role="button" alt="Go to previus page"
                                         className="ignore-global"/>
                                </picture>
                            </div>
                            <div className={ currentPage === 0 ? 'end left' : "left" } onClick={ () => handleClick(0) }>
                                <picture>
                                    <source srcSet="images/graph-arrow.svf" type="image/svg"/>
                                    <img src="images/graph-arrow.png" role="button" alt="Go to next page"
                                         className="ignore-global"/>
                                </picture>
                            </div>
                        </div>
